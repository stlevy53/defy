...
**Playable prototype — v0.1.5.** The headless rules engine (Phase 2) is **complete and rulebook-verified**, and a **playable, polished, themed React UI** (Phase 3) sits on top of it — run `npm run dev` and play a full game start to finish. **Every card renders its real printed art** (all 61 images: 24 Maquis, 20 Missions, 8 Enemy types, 8 Civilians, the Spy), over a wooden **tabletop** theme, with a **Board size** setting that scales the table 100–160% so the printed card text reads at board scale, a **settings menu** (⚙ / Esc) holding **save & load**, and a **full-card decision window** for choices that pull cards from a pile (Revealed-pile picks, deck peeks, reorder) — see [`docs/DECISION_MODAL_SPEC.md`](./docs/DECISION_MODAL_SPEC.md). **147/147 tests pass**; `tsc` + `build` are clean.
...
Board readability at that art's scale is handled by a **Board size** setting (Settings, or Ctrl +/−/0) that scales the whole table from 100% to 160%. Remaining work: making a big window pay off (the board is still capped at 1260px — see [`docs/UX_BACKLOG.md`](./docs/UX_BACKLOG.md)), the face-down Enemy card back, minor polish, an optional draft-variant setup, and further Phase 4 packaging. For the current working state and a session bootstrap, read [`HANDOFF.md`](./HANDOFF.md); for the phased roadmap see [`RESIST_PC_PORT_PLAN.md`](./RESIST_PC_PORT_PLAN.md); for the art pipeline see [`tools/card-art.md`](./tools/card-art.md).
...
4. 🔨 **Phase 3** — playable prototype UI (functional, polished, themed, with real card art in and a Board size setting to read it; making a large window pay off is the remaining piece)
