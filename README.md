...
**Playable prototype — v0.2.0.** The headless rules engine (Phase 2) is **complete and rulebook-verified**, and a **playable, polished, themed React UI** (Phase 3) sits on top of it — run `npm run dev` and play a full game start to finish. **Every card renders its real printed art**, including the **face-down Enemy back** (all 61 faces + the back: 24 Maquis, 20 Missions, 8 Enemy types, 8 Civilians, the Spy), over a wooden **tabletop** theme. The board is now **rebuilt around fixed-height bands** (status bar, phase guidance, a fixed four-across Mission row, committed lanes, hand) so a maximized window shows the whole table with far less scrolling, Missions render at a readable 452px with a **fixed five-slot garrison strip** and taught **strike order**, feedback is **consolidated** — one Attack Strength number, one event line, hover-peek instead of right-click zoom for hand/committed cards — plus a **Board size** setting that scales the table 100–160% for the printed card text, a **settings menu** (⚙ / Esc) holding **save & load**, a **full-card decision window** for pile choices — see [`docs/DECISION_MODAL_SPEC.md`](./docs/DECISION_MODAL_SPEC.md) — a **first-run coach**, the rulebook **draft setup** (offered at the start of a new game; turn it off in Settings), and **sound** (card flip, combat, end-game; mute/volume in Settings — [`docs/AUDIO_SPEC.md`](./docs/AUDIO_SPEC.md)). Spec: [`docs/COACH_SPEC.md`](./docs/COACH_SPEC.md). Tests, `tsc`, and `build` are clean.

**Automated testing.** An in-repo test harness (`sim/`) self-plays the pure engine to find bugs fast: a headless **invariant fuzzer** (`npm run fuzz`) plays thousands of seeded games per minute asserting card-conservation, softlock, crash, and unimplemented-effect oracles; a **regression corpus** (`npm run regress`) detects any behavioral change against a committed baseline; and a **live CDP harness** (`npm run tier2`) drives the real Electron build to screenshot the UI and capture console errors. See [`sim/README.md`](./sim/README.md). Every fuzz run is clean, and the regression corpus confirms the board-redesign work changed presentation only.

Board readability at that art's scale is handled by a **Board size** setting (Settings, or Ctrl +/−/0) that scales the whole table from 100% to 160%, and the board itself now uses a maximized window's width instead of capping at a fixed pixel size (see **Resolved** in [`docs/UX_BACKLOG.md`](./docs/UX_BACKLOG.md)). Remaining work is minor polish. **Sound is in** (mute/volume in Settings; table and combat cues — [`docs/AUDIO_SPEC.md`](./docs/AUDIO_SPEC.md), credits in [`docs/audio-credits.md`](./docs/audio-credits.md)). For the current working state and a session bootstrap, read [`HANDOFF.md`](./HANDOFF.md); for the phased roadmap see [`RESIST_PC_PORT_PLAN.md`](./RESIST_PC_PORT_PLAN.md); for the art pipeline see [`tools/card-art.md`](./tools/card-art.md).
