...
- 🔜 **Phase 2 — rules engine** (in progress). Slice 1 (state + RNG + setup) done. Slice 2 (PLAN core: action/decision system, play-Maquis, choose-mission) done. Slice 3 (PLAN card-action effects) done. ATTACK slice underway: sub-slice 1 (ATTACK entry + mandatory play-out) done; **sub-slice 2 (attack resolution: DEFEND queue, Attack Strength, SpendAttackOn, DEFEAT/SURVIVE, AdvancePhase → AFTERMATH) done**. **Next: ATTACK sub-slice 3 — the ATTACK-side Maquis effects + mission/enemy effects, then the deferred emilio/pilar·revealed.**
...
- `types.ts` / `setup.ts` — also gained `GameState.attackStrength` (banked base attack, spent by SpendAttackOn; reset in RECOVER — not yet built) and `MissionSlot.defeated`.
...
**Sub-slice 3 — NEXT:**
- **Implement the ATTACK-side Maquis effects** (registry ids `maquis:{id}:{side}`) still on `[stub]`: attack-value modifiers (Soledad/Abel +1 per revealed; Marcelino +1 per other; Abel +1 per civilian) — these just `state.attackStrength += bonus` when used; enemy discard/move (Anastasio, Emilio·r, Adela, Paquita·r, Consuelo·r, Adolfo·h, Soledad/Adela counter-guerrilla sweep); defense modifiers (Benigno −1, Ricardo half-mission) — mutate `enemy.defense` (Benigno) directly so `effectiveDefense` keeps reading the live value; Ricardo's mission-halving needs a per-round mission-Defense override (add a field). Draw-on-ATTACK sides (Nicolás·h, Ricardo·h) reuse the `drawHidden` helper in `effects/plan.ts` — consider promoting shared helpers to `effects/shared.ts`.
