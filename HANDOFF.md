...
- 🔜 **Phase 2 — rules engine** (in progress). Slice 1 (state + RNG + setup) done. Slice 2 (PLAN core: action/decision system, play-Maquis, choose-mission) done. Slice 3 (PLAN card-action effects) done. ATTACK slice underway: sub-slice 1 (ATTACK entry + mandatory play-out) done; **sub-slice 2 (attack resolution) done; sub-slice 3 chunks 1–2 (ATTACK Maquis effects) + chunk 3a (enemy effects + Guard/Grunt constraints) done**. **Next: sub-slice 3 chunk 3b — the 20 mission effect handlers (retires the last of the `[stub]` path).**
...
- `effects/enemies.ts` — enemy effect handlers (`registerEnemyEffects` / `ENEMY_EFFECTS`), self-filtered on `ctx.args.trigger`. Guard/Grunt are structural — see `isTargetLegal` in `actions.ts` (feeds `legalActions` + `applySpendAttackOn`).
...
- ✅ **Chunk 3a — enemy effects + ordering constraints (`effects/enemies.ts`, `registerEnemyEffects`).** SURVIVE: Counter-Guerrilla (draw a Civilian into the Graveyard), Military (shuffle hidden Maquis in play, remove one → `removedFromGame`), Spy Master (add a Spy to the Hidden discard, bounded by `spiesAvailable`), Radio Operator (place a face-down Enemy on each other Mission from the Enemy deck). DEFEAT: Jailor (Recruit top → Hidden discard). DEFEND: Engineer (+1 Defense to non-Engineers at the mission). Guard/Grunt DEFEND are **structural ordering constraints** in `actions.ts` (`isTargetLegal`: Grunts before other Enemies, Guards before the Mission) wired into both `legalActions` and `applySpendAttackOn` — their handlers are registered no-ops. Handlers self-filter on `ctx.args.trigger` (the queue fires every enemy for every trigger; each acts only on its own keyword). Tests: `effects/enemies.test.ts` (8).
- ⬜ **Chunk 3b — the 20 mission effect handlers** (`mission:{id}`), branching on `ctx.args.trigger`. Read `data/missions.json` for each keyword/effect. Some mission DEFEND effects may be ongoing constraints (handle like Guard/Grunt) vs one-shot; check each.
...
Register real effects at bootstrap with `registerPlanEffects()` + `registerAttackEffects()` + `registerEnemyEffects()` (chunk 3b adds `registerMissionEffects()`); the engine does not auto-register on import.
