...
Last updated at repo state: `main` @ `e1486f1` (Phase 2 ATTACK sub-slice 2: attack resolution shipped). Re-pin this hash after each user-side commit.
...
- 🔜 **Phase 2 — rules engine** (in progress). Slice 1 (state + RNG + setup) done. Slice 2 (PLAN core: action/decision system, play-Maquis, choose-mission) done. Slice 3 (PLAN card-action effects) done. ATTACK slice underway: sub-slice 1 (ATTACK entry + mandatory play-out) done; **sub-slice 2 (attack resolution) done; sub-slice 3 chunk 1 (attack/defense modifier effects) done**. **Next: sub-slice 3 chunk 2 — enemy manipulation + ATTACK draws.**
...
- `effects/attack.ts` — ATTACK-side Maquis effects (chunk 1: attack/defense modifiers). `registerAttackEffects()` / `ATTACK_EFFECTS`, explicit registration like the PLAN effects.
...
- ⬜ **Chunk 2 — enemy manipulation + ATTACK draws** still on `[stub]`: enemy discard (Anastasio·h/·r, Emilio·r, Adolfo·h — one enemy), Paquita·r (two), Consuelo·r (discard one + gain its Defense as Attack), Adela·h (move an enemy to another Mission), Soledad·r/Adela·r (discard all Counter-guerrillas), and ATTACK draws Nicolás·h/Ricardo·h (reuse `drawHidden` from `effects/plan.ts` — consider promoting shared helpers to `effects/shared.ts`).
- ⬜ **Chunk 3 — mission/enemy effect handlers** (`mission:{id}`, `enemy:{typeId}`), branching on `ctx.args.trigger`. This retires the `[stub]` path.
